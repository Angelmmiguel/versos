...
const buildUrlForDate = () => {
  ...
  const month = (date.getUTCMonth() + 2).toString().padStart('2', '0');
...
const setPoem = (data) => {
  q('.🖋__👂').innerHTML = stripHTML(data.attributes.title);
  q('.🖋__✍️').innerHTML = stripHTML(data.attributes.author);
  q('.🖋__📄').innerHTML = stripHTML(data.body);
}
